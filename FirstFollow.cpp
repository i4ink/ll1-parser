...
void solve(map<pair<string,string>,string> &m)
{
  vector<string>st;
    int flag=0;
    pair<string,string>key;
    vector<string>l;
    string s,exp,s_top,chr="";
    st.push_back("$");                             //////create st as vector and will deal with it like stack and added $ for string ending 
    cout<<"Enter the Starting sybmbol:";
    cin>>s;
    st.push_back(s);
    cout<<"Enter the expression:";                            ////// taken input in s for the expression
    cin>>exp;
    s="";
    for(auto x:exp)                                 ///////tokenising each symbol in l for comparing and operation
        { 
            if(!isalpha(x))
            {   l.push_back(s);
                l.push_back(&x);
        ...
        l.push_back(s);
    l.push_back("$");
    exp="$";                                       ///// at the $ for the end of the expression
    ...
    // cout<<i;
    // cout<<"\n";
    while(st.size() && l.size())                             
    {   s=l[0];
        s_top=st[st.size()-1];                     ///getting the first val from stack and string 
       // cout<<"exp:"<<s<<" stack:"<<s_top<<"\n";
       // cout<<s.size()<<" "<<s_top.size()<<"\n";
        key=make_pair(s_top,s);                                ////creating key for the findthe val from parse table
       
        if(s==s_top)                                     ///if the val from stack and string matches then delete them
        { //  cout<<"inside\n";
            st.pop_back();
            l.erase(l.begin());
        }
        else if(m.find(key)!=m.end())                                  ////else if the val of the stack exist in table take the val in stack
        {
            s=m[key];
            st.pop_back();                                          ////remove the searched key value from stack and putting its vlue from parse table
            if(all_of(s.begin(),s.end(),&::islower))
            {
                if(s!="#")                           ////if the value form parse table is not epsilon then put it over the stack other wise the key value is already deleted from stack
                st.push_back(s);        
            }
            else{                                                   ///if the val from parse table having more non terminal expression then tokenising them and put them into stack in reverse order 
                reverse(s.begin(),s.end());
              //  cout<<"string"<<s<<" "<<s.size()<<"\n";
                for(unsigned int i=0;i<s.size();i++)
                {
                    if(!isalpha(s[i]))                                                                /////handling the symbol having non alphabetic val 
                    {
                        chr=s[i];
                        if(s[i+1])
                        {
                  ...
                  ///  cout<<"after reverse:"<<chr<<" "<<chr.size()<<"\n";
                    st.push_back(chr);
                }   
            }
        }
        else{                                             //// if the value does not exist in parse table then its not valid string 
            cout<<"Invalid string";
            flag=1;
            break;
        }
    ...
    if(!flag)                                                /// if all goes right the string is valid else not 
    {
        if(!st.size() && !l.size())
        cout<<"valid string";
        else
